...
```mermaid
graph TD
    A[4 Days Prior to Admission] -->|Last Heroin Use| B(Patient at Home)
    C[1 Day Prior to Admission] -->|Ingested 4g Ibogaine + 2g Booster| B
    B -->|Found Unresponsive 8 hrs later| D[Emergency Services Arrival]
    D -->|Patient in Asystole / Covered in Emesis| E[Field Treatment]
    E -->|Intubation + 2mg Naloxone + CPR| F[Transport]
    F -->|ROSC Achieved| G[Community Hospital]
    G -->|Transfer| H[Tertiary Care Institution]
    H -->|Brain Death Confirmed| I[Cardiopulmonary Death]
```
