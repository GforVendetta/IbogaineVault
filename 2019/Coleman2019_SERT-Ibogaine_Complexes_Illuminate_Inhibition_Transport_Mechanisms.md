...
key_findings: "NATURE LANDMARK: First cryo-EM SERT-ibogaine structures showing outward-open, occluded, inward-open conformations. Defines transport mechanism and ibogaine non-competitive inhibition at molecular level."
...
source_pdf: "2019/Coleman2019_SERT-Ibogaine_Complexes_Illuminate_Inhibition_Transport_Mechanisms.pdf"
...
```mermaid
graph TD;
    A[2,904 micrographs] -->|DoG Picker| B[1,278,876 particles]
    B -->|Rounds of 2D classification| C[431,071 particles]
    C -->|Ab initio in cryoSPARC| D[2 classes found]
    D -->|Class 1: 49.8%<br>214,537 particles| E[cryoSPARC refinement<br>without mask]
    D -->|Class 2: 50.2%<br>216,534 particles| D2[Discarded]
    E --> F[FSC resolution = 5.9 Å<br>214,537 particles]
    F -->|Recentering particles<br>cisTEM local refinement<br>Masking out constant domain of Fab and micelle| G[FSC resolution = 4.3 Å]
    G -->|Phenix local sharpening<br>B factor = -147 Å^2| H[Final Map]
```
...
```mermaid
graph TD;
    A[1,639 micrographs] -->|DoG Picker| B[592,117 particles]
    B -->|Rounds of 2D classification| C[153,986 particles]
    C -->|Ab initio in cryoSPARC| D[Initial Model]
    D -->|cryoSPARC refinement<br>without mask| E[FSC resolution = 5.2 Å<br>153,986 particles]
    E -->|Recentering particles<br>local ctf refinement<br>cryoSPARC refinement<br>without mask| F[FSC resolution = 5.1 Å<br>153,986 particles]
    F -->|cisTEM local refinement<br>Masking out constant domain of Fab and micelle| G[FSC resolution = 4.1 Å]
    G -->|cisTEM local sharpening<br>B factor = -400 Å^2| H[Final Map]
```
...
```mermaid
graph TD;
    A[10,632 micrographs] -->|DoG Picker| B[2,615,403 particles]
    B -->|Rounds of 2D classification| C[724,394 particles]
    C -->|Ab initio in cryoSPARC| D[Initial Model]
    D -->|Relion refinement<br>recentering particles<br>cryoSPARC refinement<br>without mask| E[FSC resolution = 6.2 Å<br>724,394 particles]
    E -->|Recentering particles<br>cisTEM local refinement<br>Masking out constant domain of Fab and micelle| F[FSC resolution = 4.2 Å]
    F -->|cisTEM local sharpening<br>B factor = -90 Å^2| G[Final Map]
```
...
```mermaid
graph TD;
    A[7,732 micrographs] -->|DoG Picker| B[1,220,861 particles]
    B -->|Rounds of 2D classification| C[383,617 particles]
    C -->|Ab initio in cryoSPARC| D[Initial Model]
    D -->|Relion 3D classification<br>recentering particles| E{Classification}
    E -->|0.02%| E1[Discarded]
    E -->|2.4%| E2[Discarded]
    E -->|1.36%| E3[Discarded]
    E -->|96.1%<br>368,656 particles| F[Selected Class]
    E -->|0.12%| E5[Discarded]
    E -->|0.02%| E6[Discarded]
    F -->|cryoSPARC refinement<br>without mask| G[FSC resolution = 4.9 Å]
    G -->|Recentering particles<br>cisTEM local refinement<br>Masking out constant domain of Fab and micelle| H[FSC resolution = 3.6 Å]
    H -->|cisTEM local sharpening<br>B factor = -150 Å^2| I[Final Map]
```
...
```mermaid
graph TD;
    A[Shifting ibogaine at the central binding site<br>showing center of mass only] --> B[7.5 M initial poses of ibogaine];
    B --> C[1. Minimization<br>2. RMSD clustering of favored poses negative PIE];
    C --> D{Number of<br>resulting clusters<br>converged?};
    D -->|NO| C;
    D -->|YES| E[20 clusters for outward-open<br>30 for occluded<br>46 for inward-open];
    E --> F[10-ns MD simulation for the pose<br>with strongest PIE from each cluster];
    F --> G[Collective analysis of the snapshot sets of ibogaine poses];
    G --> H[Optimal binding poses];
```
...
[ ] | [x] The statistical test(s) used AND whether they are one- or two-sided<br>Only common tests should be described solely by name; describe more complex techniques in the Methods section.
...
[ ] | [x] Clearly defined error bars<br>State explicitly what error bars represent (e.g. SD, SE, CI)
