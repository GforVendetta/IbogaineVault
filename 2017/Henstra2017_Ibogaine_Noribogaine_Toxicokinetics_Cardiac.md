---
title: "Toxicokinetics of ibogaine and noribogaine in a patient with prolonged multiple cardiac arrhythmias after ingestion of internet purchased ibogaine"
authors:
  - "Henstra, Marieke"
  - "Wong, Liza"
  - "Chahbouni, Abdel"
  - "Swart, Noortje"
  - "Allaart, Cor"
  - "Sombogaard, Ferdi"
year: 2017
category: RED
tags:
  - topic/cardiac
  - topic/pharmacokinetics
  - topic/noribogaine
  - topic/adverse-event
  - method/case-report
key_findings: "Noribogaine, not ibogaine, drives prolonged cardiac toxicity. QTc 647 ms with TdP. Noribogaine t½ elimination 2540 h vs ibogaine 228 h. k₁₂/k₂₁ ratio 21.5 for noribogaine implies deep tissue sequestration."
source_pdf: "2017/Henstra2017_Ibogaine_Noribogaine_Toxicokinetics_Cardiac.pdf"
doi: "10.1080/15563650.2017.1287372"
pmid: "28489458"
journal: "Clinical Toxicology"
issn: "1556-9519"
publication_date: "2017-02-09"
document_type: case-report
secondary_categories: [ORANGE]
clinical_significance: high
aliases:
  - "Henstra 2017"
  - "Ibogaine Noribogaine Toxicokinetics Cardiac Arrhythmias"
evidence_level: case-report
sample_size: 1
qtc_data: true
electrolyte_data: true
herg_data: false
contraindications:
  - "Pre-existing cardiac conditions"
  - "Concurrent CYP2D6-metabolised medications"
  - "Electrolyte imbalances (hypokalaemia, hypomagnesaemia)"
dosing_range: "1400 mg oral (repeated doses over 12 hours, internet-sourced capsules)"
route: oral
open_access: false
publisher: "Taylor & Francis"
body_format: vault-analytical
---

# Toxicokinetics of Ibogaine and Noribogaine in a Patient with Prolonged Multiple Cardiac Arrhythmias After Ingestion of Internet Purchased Ibogaine

**Citation:** Henstra, M., Wong, L., Chahbouni, A., Swart, N., Allaart, C., & Sombogaard, F. (2017). Toxicokinetics of ibogaine and noribogaine in a patient with prolonged multiple cardiac arrhythmias after ingestion of internet purchased ibogaine. *Clinical Toxicology*, 55(6), 600–602. https://doi.org/10.1080/15563650.2017.1287372

## Abstract

Ibogaine is an agent evaluated as an unapproved anti-addictive agent for the management of drug dependence. Sudden cardiac death has been described secondary to its use. The authors describe the clinical effects and toxicokinetics of ibogaine and noribogaine in a single patient, developing an LC-MS/MS method to measure ibogaine and noribogaine plasma concentrations using two compartments with first-order absorption.

The maximum concentration of ibogaine was 1.45 mg/L. The patient developed a markedly prolonged QTc interval of 647 ms maximum, with several cardiac arrhythmias including atrial tachycardia, ventricular tachycardia, and Torsades de Pointes (TdP). QTc prolongation remained present until 12 days after ingestion, several days after ibogaine plasma levels were low, implicating clinically relevant noribogaine concentrations long after ibogaine clearance. The ratio k₁₂/k₂₁ for noribogaine was 21.5 and 4.28 for ibogaine, implicating lower distribution of noribogaine from the peripheral compartment into the central compartment compared to ibogaine.

A direct relationship between noribogaine concentration and long duration of cardiac action was demonstrated, rather than with parent ibogaine.

## Key Findings

- **QTc prolongation of 647 ms** — markedly prolonged, with multiple arrhythmias including atrial tachycardia, ventricular tachycardia, and TdP
- **Noribogaine drives prolonged cardiac toxicity** — QTc prolongation persisted until 12 days post-ingestion, well after ibogaine plasma levels had fallen to negligible levels
- **Extreme noribogaine elimination half-life:** t½,₂ = 2,540 h (vs 228 h for ibogaine); far exceeding the 28–49 h reported by Glue et al. (2015) in healthy volunteers, likely due to higher Cmax in this poisoning case
- **Deep tissue sequestration of noribogaine:** k₁₂/k₂₁ ratio of 21.5 (vs 4.28 for ibogaine) indicates substantially greater net transfer into peripheral compartments
- **Electrolyte disturbances:** Hypokalaemia (3.4 mmol/L) and hypomagnesaemia (0.68 mmol/L) developed, both of which increase TdP risk
- **Enterohepatic recirculation** suggested by fluctuating noribogaine plasma concentrations, potentially contributing to prolonged elimination

## Methodology

Single-patient case report with toxicokinetic modelling. A validated LC-MS/MS method was developed to measure ibogaine and noribogaine plasma concentrations (quantification range 10–2,000 µg/L; LLOQ 3 µg/L for both analytes). Toxicokinetics were modelled using two compartments with first-order absorption without lag-time, analysed with MW/Pharm 3.60 (MediWare, Czech).

## Case Presentation

A 46-year-old woman presented to the emergency department after being found unconscious following repeated ingestion over 12 hours of internet-purchased ibogaine capsules (total dose: 1,400 mg), taken to curb withdrawal symptoms from heroin addiction. Past medical history included alcohol, cocaine, and cannabis addiction; no cardiovascular disease or chronic medication.

**On admission:** Conscious, breathing independently; pulse 85/min; blood pressure 126/86 mmHg. Physical examination unremarkable. Urine toxicology screen negative for co-ingestants.

**Cardiac events:**
- ECG: QTc 647 ms, atrial tachycardia, ventricular tachycardia
- TdP developed before transfer to coronary care unit (spontaneously terminated)
- Isoproterenol started intravenously to suppress TdP initiation
- Despite isoproterenol, TdP recurred → temporary pacemaker inserted (rate 130/min)
- Day 1: Hypokalaemia (3.4 mmol/L) and hypomagnesaemia (0.68 mmol/L) → IV supplementation
- With pacing and electrolyte correction, no further arrhythmias occurred
- Pacing rate gradually reduced to 100/min over 72 h; pacemaker lead removed after 5 days
- QTc prolongation persisted until 12 days post-ingestion

## Toxicokinetic Parameters

| Parameter | Ibogaine | Noribogaine |
|-----------|----------|-------------|
| C_max | 1.45 mg/L (at 12.7 h) | 0.569 mg/L (at 98 h) |
| CL/F (oral clearance) | 4.66 L/h | 2.30 L/h |
| V₁ (central compartment) | 185 L | 47.5 L |
| V₂ (peripheral compartment) | 789 L | 1,022 L |
| t½,₁ (distribution) | 26.06 h | 12.59 h |
| t½,₂ (elimination) | 228 h | 2,540 h |
| k₁₀ (elimination rate) | 0.0079 /h | — |
| k₁₂ | 0.0014 /h | 0.0067 /h |
| k₂₁ | 0.0003 /h | 0.0003 /h |
| k₁₂/k₂₁ ratio | 4.28 | 21.5 |
| Calculated logP (pH 7.4) | 2.19 | 2.03 |

## Clinical Implications

This case is profoundly relevant to clinical clinical protocols in several ways:

**Monitoring duration:** QTc prolongation persisted for 12 days post-ingestion — far longer than the pharmacokinetic profile of ibogaine alone would suggest. This underscores the need for extended cardiac monitoring well beyond ibogaine clearance, with noribogaine as the true driver of prolonged risk. Standard 24–72 h monitoring windows may be insufficient for patients with high Cmax or suspected CYP2D6 polymorphisms.

**Electrolyte vigilance:** The development of hypokalaemia and hypomagnesaemia during treatment, despite normal baseline values, reinforces the critical importance of continuous electrolyte monitoring and proactive supplementation — a principle now formalised in the MISTIC protocol's magnesium cardioprotection paradigm.

**CYP2D6 genotyping:** The authors note they did not perform CYP2D6 genotyping. Given that CYP2D6 polymorphism directly influences ibogaine-to-noribogaine conversion rates and thus both efficacy and cardiac risk profile, pre-treatment genotyping remains clinically valuable for risk stratification.

**Noribogaine tissue sequestration:** The k₁₂/k₂₁ ratio of 21.5 for noribogaine (vs 4.28 for ibogaine) indicates dramatically greater net transfer into peripheral tissue compartments — functioning like a slow-release reservoir that continues to feed noribogaine back into central circulation. This has direct implications for discharge timing decisions.

## Limitations

- Single-patient case report — toxicokinetic parameters may not generalise
- CYP2D6 genotyping was not performed, precluding assessment of metaboliser status
- The extremely long noribogaine t½,₂ (2,540 h) may reflect the unusually high Cmax in this poisoning case rather than typical elimination kinetics
- Exact contribution of electrolyte disturbances to arrhythmogenesis cannot be isolated from noribogaine-mediated QTc prolongation
- Internet-purchased ibogaine — purity and exact composition unverified

---

## See Also
- [[2012/Alper2012_Ibogaine_Fatalities]]
- [[2016/Litjens2016_How_Toxic_Is_Ibogaine]]
- [[2015/Glue2015_Noribogaine_Ascending_Doses]]
- [[2017/Rubi2017_hERG_iPSC_Cardiomyocytes]]
- [[RED_Cardiac_Safety_Hub]]
